...
namespace kuakling\materialize\widgets;
...
class LinkPager extends Widget
{
    ...
    protected function renderPageButton($label, $page, $class, $disabled, $active)
    {
        ...
        if ($disabled) {
            ...
            return Html::tag(
                'li', 
                Html::a(Html::tag('i', $label, ['class'=>'material-icons']),'#!'),
                $options
            );
        }else{
        ...
        if($label == $this->nextPageLabel || $label == $this->prevPageLabel){
            $label = Html::tag('li', Html::tag('i', $label, ['class'=>'material-icons']), $options);
        }

        $liContent = Html::a($label, $this->pagination->createUrl($page), $linkOptions);
        if ($active) {
            $liContent = Html::a($label, '#!', $linkOptions);
        }

        return Html::tag('li', $liContent, $options);
    }
